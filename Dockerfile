...
ENV BUILD_TIMESTAMP='20161209_01' FIREFOX_VERSION=${FIREFOX_VERSION} CHROME_VERSION=${CHROME_VERSION} X11_RESOLUTION=${X11_RESOLUTION} VNC_PASSWD=${VNC_PASSWD}
...
RUN apt-get update -y --fix-missing -qq \
  ...
  && apt-get install -y wget libgtk-3-0 openjdk-8-jdk maven openssh-client git vim xvfb x11vnc \
